...
void DrawDebugLine(const sf::Vector2f& pos, const sf::Vector2f& dir, const float& norm, sf::RenderTarget& target, const sf::Color& colour)
{
...
const sf::Vector2f CalcIntersectionPoint(const sf::Vector2f& x1, const sf::Vector2f& x2, const sf::Vector2f& y1, const sf::Vector2f& y2)
{
...
bool ptContatinedInPoly(sf::Vector2f testPt, const sf::ConvexShape& poly)
{
	...
	for (i = 0, j = nvert-1; i < nvert; j = i++) {
		if ( ((poly.getPoint(i).y > testPt.y) != (poly.getPoint(j).y > testPt.y)) && (testPt.x < (poly.getPoint(j).x - poly.getPoint(i).x) * (testPt.y - poly.getPoint(i).y) / (poly.getPoint(j).y - poly.getPoint(i).y) + poly.getPoint(i).x) )
