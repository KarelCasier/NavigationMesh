...
public:
    ...
    sf::Vector2f getMousePositionRelativeToWorldView()
    {
        return mWindow.mapPixelToCoords(sf::Mouse::getPosition(mWindow), mWorldView);
    }

    sf::ConvexShape* createCircle(const int& numPts,
                                  const float& radius,
                                  const int& rotationDeg,
                                  const sf::Vector2f& location);
    sf::ConvexShape* createRectangle(const float& width,
                                     const float& height,
                                     const int& rotationDeg,
                                     const sf::Vector2f& location);
