        ...
        for (int j = i; j < mNavPolys.size(); j++) {
            ...
            for (int u = 0; u < iPoly->mPoly->getPointCount(); u++) {
                for (int v = 0; v < jPoly->mPoly->getPointCount(); v++) {
                    if (iPoly->mPoly->getPosition() + iPoly->mPoly->getPoint(u) ==
                        jPoly->mPoly->getPosition() + jPoly->mPoly->getPoint(v)) {
                        ++numPtsConnected;
            ...
            if (numPtsConnected > 3) {
                std::cout << "Alert: 2 Polygons touch by more then 2 points which may cause navigation problems"
                          << std::endl;
            }
...
{
    return fabs(a->mPoly->getPosition().x - b->mPoly->getPosition().x) +
           fabs(a->mPoly->getPosition().y - b->mPoly->getPosition().y);
}

std::vector<sf::Vector2f> NavMesh::RecreatePathPoints(std::unordered_map<NavPoly*, NavPoly*> CameFrom,
                                                      NavPoly* current,
...
std::vector<NavPoly*> NavMesh::RecreatePathNavPolys(std::unordered_map<NavPoly*, NavPoly*> CameFrom,
                                                    NavPoly* current,
                                                    const sf::Vector2f& startPos,
                                                    const sf::Vector2f& targetPos)
{
...
bool NavMesh::HasLineOfSight(const sf::Vector2f& a, const sf::Vector2f& b)
{
    ...
    for ( auto neighbourPair : aNavPoly->mNeighbours ) {
            if (neighbourPair.first == bNavPoly && doIntersect(a, b, neighbourPair.second.start,
    neighbourPair.second.end)) { result = true;
            }
    }
    return result;
    //*/
...
std::vector<sf::Vector2f> NavMesh::OptimizePathFunnel(const std::vector<NavPoly*>& polyPath,
                                                      ...
                                                      const sf::Vector2f& target)
{
    ...
    for (int i = 2; i < polyPath.size(); ++i) {
        ...
        // ); < Advance to next edge < Get neighbour and edge
        ...
        if (left != neighEdge.start) {
            ...
            // float potenAngle = acosf( DotProduct(potenLeft - apex, right - apex) / ( Norm(potenLeft, apex) *
            // Norm(right, apex) ) );
...
std::vector<sf::Vector2f> NavMesh::GetNavPath(const sf::Vector2f& startPos, const sf::Vector2f& targetPos)
{
        ...
        while (!open.empty()) {
                ...
                for (auto next : currentNavPoly->mNeighbours) {
                        ...
                        float nextCost = cost_so_far[currentNavPoly] + 1.0/heuristic(GetCenter(*currentNavPoly->mPoly),
GetCenter(*next.first->mPoly));
