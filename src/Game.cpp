...
, mUIView(sf::Vector2f(WINDOWWIDTH, WINDOWHEIGHT) / 2.f,
          sf::Vector2f(WINDOWWIDTH, WINDOWHEIGHT)) ///< View used to render UI
, camPos(0, 0)
...
void Game::loadMap()
{
        ...
        if (((std::string) "walls").compare(pObjectGroup->Attribute("name")) == 0) {
            ...
            while (pEle) {
                ...
                if (subObject) {
                    ...
                    if (typeEle) {
                        sf::ConvexShape* poly =
                            createPolygon(typeEle->Attribute("points"), pEle->IntAttribute("rotation"),
                                          sf::Vector2f(pEle->FloatAttribute("x"), pEle->FloatAttribute("y")));
                        shapes.push_back(poly);
                    }
                    typeEle = pEle->FirstChildElement("ellipse");
                    if (typeEle) {
                        sf::ConvexShape* circle =
                            createCircle(64, pEle->FloatAttribute("width") / 2.f, pEle->IntAttribute("rotation"),
                                         sf::Vector2f(pEle->FloatAttribute("x"), pEle->FloatAttribute("y")));
                        shapes.push_back(circle);
                    }
                } else ///< Regular rectangle
                {
                    sf::ConvexShape* rect = createRectangle(
                        pEle->FloatAttribute("width"), pEle->FloatAttribute("height"), pEle->IntAttribute("rotation"),
                        sf::Vector2f(pEle->FloatAttribute("x"), pEle->FloatAttribute("y")));
    ...
    for (sf::ConvexShape* shape : shapes) {
        ...
        for (auto i = 0; i < ptCnt-1; ++i)
        {
                ...
                newNormal->pos = shape->getPosition() + shape->getPoint(i) + (shape->getPoint(i+1) - shape->getPoint(i))
        / 2.f; newNormal->normal = CalcEdgeNormal(shape->getPoint(i), shape->getPoint(i+1));
                normals.push_back(newNormal);
        }

        Normal* newNormal = new Normal();
        newNormal->pos = shape->getPosition() + shape->getPoint(ptCnt-1) + (shape->getPoint(0) -
        shape->getPoint(ptCnt-1)) / 2.f; newNormal->normal = CalcEdgeNormal(shape->getPoint(ptCnt-1),
        shape->getPoint(0)); normals.push_back(newNormal);
        */
...
sf::ConvexShape* Game::createCircle(const int& numPts,
                                    const float& r,
                                    const int& rotationDeg,
...
sf::ConvexShape* Game::createRectangle(const float& width,
                                       const float& height,
                                       const int& rotationDeg,
                                       const sf::Vector2f& location)
{
...
void Game::update(sf::Time dTime)
{
    ...
    //
    ///< Player Collisions
    std::vector<sf::ConvexShape*> potentialCollisionShapes =
        quadTree.GetObjectsAt(mPlayer.getPosition().x, mPlayer.getPosition().y);

    for (auto shape : potentialCollisionShapes) {
        shape->setFillColor(sf::Color::Red);
        int numPts = (int)shape->getPointCount();
        for (int i = 0; i < numPts - 1; ++i) {
            if (doIntersect(prevPos, nextPos, shape->getPosition() + shape->getPoint(i),
                            shape->getPosition() + shape->getPoint(i + 1))) {
                ...
                sf::Vector2f u((DotProduct(mPlayer.getVelocity(), shapeNormal) / DotProduct(shapeNormal, shapeNormal)) *
                               shapeNormal);
                mPlayer.setVelocity(mPlayer.getVelocity() - u);
            }
        }
        if (doIntersect(prevPos, nextPos, shape->getPosition() + shape->getPoint(numPts - 1),
                        shape->getPosition() + shape->getPoint(0))) {
...
void Game::updateStatistics(sf::Time elapsedTime)
{
    ...
    if (mStatisticsUpdateTime >= sf::seconds(1.0f)) {
        strStream << "Frames / Second = " << mStatisticsNumFrames << "\n"; // << "Time / Update = " <<
                                                                           // mStatisticsUpdateTime.asMicroseconds() /
                                                                           // mStatisticsNumFrames << "us" << std::endl;
