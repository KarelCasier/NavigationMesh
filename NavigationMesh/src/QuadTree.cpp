...
QuadTree::QuadTree(const float& x, const float& y, const float& width, const float& height, const int& _depth, const int& maxDepth)
:mX(x)
,mY(y)
,mWidth(width)
,mHeight(height)
...
{
	return	 !(object->getGlobalBounds().left	< child->mX ||
			   object->getGlobalBounds().top	< child->mY ||
