...
void ShadowManager::updateShadows(const sf::Vector2f& newLightPos)
{
	...
	for (const sf::ConvexShape* shape : mShadowCasters)
	{
		...
		for (int i = 0; i < numVerticies; i++)
		{
			...
			{
				sf::Vector2f edgeIntersection = GetIntersectionPoint(lightPos, minimalIntersection, shape->getPoint(j) + shapePos, shape->getPoint(j+1) + shapePos);
				if (isPointOnLineSegment(edgeIntersection, shape->getPoint(j) + shapePos, shape->getPoint(j+1) + shapePos))
				{
			...
			sf::Vector2f edgeIntersection = GetIntersectionPoint(lightPos, minimalIntersection, shape->getPoint(numVerticies-1) + shapePos, shape->getPoint(0) + shapePos);
			if (isPointOnLineSegment(edgeIntersection, shape->getPoint(numVerticies-1) + shapePos, shape->getPoint(0) + shapePos))
			{
				sf::Vector2f offset = Normalize(v-lightPos) * 10.f;
				possiblePoints.push_back(edgeIntersection + offset);
			}
		...
		{
			if (doIntersect(lightPos, mScreenCorners[0], shape->getPoint(i) + shapePos, shape->getPoint(i+1) + shapePos))
			{
				shadowPoints.push_back(mScreenCorners[0]);
				//mDebugPoints.push_back(shadowPoints.back());
				//mDebugLines.push_back(shadowPoints.back());
			}
			if (doIntersect(lightPos, mScreenCorners[1], shape->getPoint(i) + shapePos, shape->getPoint(i+1) + shapePos))
			{
				shadowPoints.push_back(mScreenCorners[1]);
				//mDebugPoints.push_back(shadowPoints.back());
				//mDebugLines.push_back(shadowPoints.back());
			}
			if (doIntersect(lightPos, mScreenCorners[2], shape->getPoint(i) + shapePos, shape->getPoint(i+1) + shapePos))
			{
				shadowPoints.push_back(mScreenCorners[2]);
				//mDebugPoints.push_back(shadowPoints.back());
				//mDebugLines.push_back(shadowPoints.back());
			}
			if (doIntersect(lightPos, mScreenCorners[3], shape->getPoint(i) + shapePos, shape->getPoint(i+1) + shapePos))
			{
			...
			if (norm > 200)
			{
				...
				{
					sf::Vector2f pos = shape->getPoint(i) + shapePos + unitVec * (distanceBetweenRays * (float)(j));
